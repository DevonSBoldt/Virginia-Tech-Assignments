...
class TestPromptCreationRetrieval(BaseTest):
    ...
    async def _assert_prompts_equal(self, user: TestUserSession, expected_guid, expected_prompt, actual_prompt):
        ...
        assert actual_prompt['guid'] == expected_guid, f"{user_name}'s retrieved prompt GUID does not match the created prompt's GUID."
        assert actual_prompt['content'] == expected_prompt['content'], f"The content of {user_name}'s retrieved prompt does not match the expected content."
        assert actual_prompt['display_name'] == expected_prompt['display_name'], f"The display name of {user_name}'s retrieved prompt does not match the expected display name."
        assert set(actual_prompt['tags']) == set(expected_prompt['tags']), f"The tags of {user_name}'s retrieved prompt do not match the expected tags."
